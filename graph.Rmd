...
## Hue Chroma Luminance (HSL)

```{r}
library(ggplot2)

dt <- expand.grid(seq(0, 320, 40), seq(10, 90, 10), seq(0, 100, 10))
dt <- as.data.table(dt)
colnames(dt) <- c("Hue", "Chroma", "Luminance")
dt <- dt[, clr := hcl(Hue, Chroma, Luminance)]

ggplot(dt, aes(y=Chroma, x=Luminance, colour=clr)) +
    geom_point(size=3) +
    scale_color_identity() +
    scale_y_continuous(breaks=seq(10, 90, 10)) +
    scale_x_continuous(breaks=seq(0, 100, 10)) +
    facet_wrap( ~ Hue, nrow=3)
