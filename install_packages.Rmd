...
.libPaths()
pkg_name <- "pacman"
dsp <- utils::packageDescription(pkg_name)
packageDescription(pkg_name, fields = "Imports")
dsp$Depends
dsp$Imports
dsp$Suggests
names(dsp)
...
get_imports <- function(name){

  if (is.na(utils::packageDescription(name))) {invisible(NA)}

  else {dsp <- utils::packageDescription(name)

  nms <- strsplit(dsp$Imports,",| |\n")[[1]]

  nms <- nms[lapply(nms, nchar) >= 2]

  nms <- nms[lapply(nms, function(str){substring(str, 1, 1)}) != "("]

  nms <- nms[lapply(nms, function(str){substring(str, nchar(str), nchar(str))}) != ")"]
...
lim <- lapply(package_list[], utils::packageDescription)
