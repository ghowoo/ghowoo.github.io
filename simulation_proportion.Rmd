...
Estimated proportion of the stratum $h$:
$$ P_{hy} = \frac{\sum_{i=1}^{n_h}(Y_{h,i})}{n_h}$$


Variance estimate of the stratum $h$:

$$ s^2_h = \frac{\sum_{i=1}^{n_h}(x_{h,i} - \bar{x}_h)^2}{n_h - 1}$$
$$ s^2_h = \frac{n_h}{n_h - 1}P_{py}(1 - P_{py})$$
...
$$\hat{SE}(\bar{x}_{str}) = \sqrt{\sum_{h=1}^L(\frac{N_h}{N})^2\frac{s_{hx}^2}{n_h}(\frac{N_h-n_h}{N_h})} $$
