...
\newpage
    \fancyhead{}
    \fancyhead[L]{\text{Wu's R Notes}\\\text{rmarkdown Template Generating PDF}}
    \fancyhead[R]{\rightmark\\\text{Version 1.1}}
    \fancyfoot[L]{\text{Programmer:} Wu Gong \text{  } \DTMnow}
\listoftables
...
\fancyhead{}
\fancyhead[L]{\text{Wu's R Notes}\\\text{rmarkdown Template Generating PDF}}
\fancyhead[R]{\thepage\text{ of }\ztotpages\\\text{Version 1.1}}
\fancyfoot{}
\fancyfoot[L]{\text{Programmer:} Wu Gong \text{  } `r format(Sys.time(), "%d %b %Y %H:%M")`}
...
group_index <- auto_index(dt$group_id)
kbl(dt
    , caption = "Spanning Multiple Pages"
    , align = c("l", "r")
    , format = "latex"
  , booktabs = TRUE
    , longtable = TRUE) %>%
    ...
    kable_styling(latex_options = c("HOLD_position", "repeat_header")
                ...
                ## , protect_latex = TRUE
                , full_width = FALSE) %>%
    footnote(general = "This is a Latex table output."
           , number = "number"
             , alphabet = "Note A")
...
dt <- data.table(id = 1:n
               ...
               , var1 = sample(1000:3000, size = n, replace = TRUE)
               , var2 = sample(100:300, size = n, replace = TRUE)
               , var3 = sample(100:300, size = n, replace = TRUE)
               , var4 = sample(100:300, size = n, replace = TRUE)
               , var5 = sample(100:300, size = n, replace = TRUE)
               , var6 = sample(100:300, size = n, replace = TRUE)
                 )
  ...
  , longtable = TRUE) %>%
    ...
    kable_styling(latex_options = c("HOLD_position", "repeat_header")
                , font_size = 12
                  ## , protect_latex = TRUE
                , full_width = FALSE) %>%
    footnote(general = c("This is a Latex table output.", "This is the second note.")
           , number = "Number 1"
           , general_title = "")
